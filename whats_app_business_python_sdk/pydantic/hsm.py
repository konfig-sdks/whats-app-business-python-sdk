# coding: utf-8

"""
    WhatsApp Business API

    See https://developers.facebook.com/docs/whatsapp

    The version of the OpenAPI document: 1.0
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING
from pydantic import BaseModel, Field, RootModel

from whats_app_business_python_sdk.pydantic.language import Language
from whats_app_business_python_sdk.pydantic.localizable_param import LocalizableParam

class Hsm(BaseModel):
    # The element name that indicates which template to use within the namespace
    element_name: str = Field(alias='element_name')

    language: Language = Field(alias='language')

    # This field is an array of values to apply to variables in the template
    localizable_params: typing.List[LocalizableParam] = Field(alias='localizable_params')

    # The namespace that will be used
    namespace: str = Field(alias='namespace')
    class Config:
        arbitrary_types_allowed = True
